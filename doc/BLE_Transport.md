...
NimBLE-Arduino 2.5.1 puede restaurar seguridad y suscripciones antes de entregar `onConnect`, porque difiere CONNECT hasta completar el intercambio de versión/features remotas. El firmware inicializa su sesión al entrar en `onConnect`, conserva la seguridad ya presente en ese descriptor y no vuelve a iniciar seguridad si cumple la política. Los eventos anteriores siguen sin autorizar comandos ni crear una sesión.

Desde el consumidor se programa una consulta al host BLE al conectar y aproximadamente cada 250 ms mientras siga conectado. Esta consulta lee el descriptor de seguridad y los **CCCD actuales de esa conexión**, no las preferencias guardadas en el bond. El trabajo captura `(handle, generación)` y se descarta si ya no corresponde a la sesión activa. Las suscripciones y la seguridad recuperadas no publican READY: el consumidor sigue completando limpieza/inicialización antes de hacerlo. Un fallo de lectura no concede disponibilidad, y la pérdida de seguridad o de suscripción TX la revoca.

La consulta es local: no envía peticiones ATT al cliente ni hace escrituras NVS. El adaptador `BLELiveConnection.h` usa una API interna de lectura ATT de NimBLE-Arduino 2.5.x, porque esta versión no expone un getter público de CCCD por conexión; hay que revisar ese adaptador al actualizar la dependencia. Los fallos de envío de estado conservan su intervalo mínimo de reintento de 250 ms, también cuando se ejecuta una reconciliación periódica. Véase [análisis del orden de conexión y pruebas](BLE_Connection_State.md).
